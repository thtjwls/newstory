<div class="views-container col-md-12">
	...
	<?=form_open_multipart('/ajax/insert',array('id' => 'upload_form'))?>
	<div class="form-group clearfix">
		<select name="nb_category" id="nb_category" class="form-control pull-left">
			<option value="NULL">선택</option>
			<? foreach($menu as $v) { if ($v['idx'] == 100000) continue; ?>
			<option value="<?=$v['idx']?>"><?=$v['name']?></option>
			<? } ?>
		</select>		
		<input type="text" id="wr_subject" name="subject" class="no-round form-control pull-right" placeholder="제목을 입력하세요."/>
	</div>
	<div class="form-group">
		<textarea name="ck_tx" id="ck_write" class="ck_write"></textarea>
	</div>
